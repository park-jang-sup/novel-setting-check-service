...
def _build_prompt(settings_text, manuscript_text, rule_findings):
    ...
    lines.append(
        "    용어 불일치는 같은 대상을 서로 다른 이름으로 부를 때만 해당한다. "
        "다음 두 경우는 용어 불일치로 보지 마라."
    )
    lines.append(
        "      - 서로 다른 대상을 가리키는 단어: 어떤 능력과 그 능력을 쓰는 도구처럼 "
        "지칭 대상이 다르면 불일치가 아니라 각각 별개의 표현이다."
    )
    lines.append(
        "      - 같은 단어에 수식어나 조사가 붙은 표현: 같은 단어에 수식어·조사만 "
        "붙은 경우는 같은 대상을 가리키는 표현으로 본다."
    )
    lines.append(
